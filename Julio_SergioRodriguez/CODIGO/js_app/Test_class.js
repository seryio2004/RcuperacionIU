...
/*
   test_entidad() {
    ...
    for (let i = 0; i < this.arrayclass test {
...
_pruebas.length; i++) {
        ...
        // Si el campo no existe, lo saltamos
        if (document.getElementById(campotest) == null) {
            console.log('No existe el campo ' + campotest);
            continue;
        }
        ...
        // En lugar de funciones tipo comprobar_name_xxx, validamos con la estructura:
        let resultado = validar.comprobarCampo(
            campotest,
            acciontest,
            this.estructura,
            this.validaciones
        ...
        let resultadoTexto = resultado ? "OK" : "ERROR";
        let comparacion = resultadoTexto == respuestatest ? "✔️" : "❌";

        salidatest += `<tr><td>${numdeftest}</td><td>${numprueba}</td><td>${campotest}</td><td>${this.array_pruebas[i][3]}</td><td>${acciontest}</td><td>${valortest}</td><td>${resultadoTexto}</td><td>${respuestatest}</td><td>${comparacion}</td></tr>`;
    }

    document.getElementById('tablaresultadostest').innerHTML = salidatest;

    // Ocultar formulario al final de la prueba
    document.getElementById('div_IU_form').style.display = 'none';
}
